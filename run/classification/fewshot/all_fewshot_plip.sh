...
print(f'losses_{losses}_weights_{weights}')" 2>/dev/null)
    ...
    for dataset in "${new_class_datasets[@]}"; do
        # Per-dataset CalibPrompt hyperparameters (PLIP, paper Table 5)
        case "${dataset}" in
            kather)     SMAC_ALPHA=0.05; AS_WEIGHT=0.01  ;;
            pannuke)    SMAC_ALPHA=0.2;  AS_WEIGHT=0.1   ;;
            digestpath) SMAC_ALPHA=0.03; AS_WEIGHT=0.001 ;;
            *)          SMAC_ALPHA=0.2;  AS_WEIGHT=0.1   ;;
        esac
        ...
        for seed in "${seeds[@]}"; do
            ...
            bash scripts/classification/all_fewshot_plip.sh ${TRAINER} ${TRAINER_CFG} ${dataset} ${DATA_DIR} ${SHOTS} ${seed} "${HP_OPTS}"
        done
